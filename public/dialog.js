...
var ieVer = isIE ? parseInt(agt.split(";")[1].replace(/(^\s*)|(\s*$)/g, "")
		.split(" ")[1]) : 0;
...
/*
 * Array.prototype.remove = function (s, dust) { //如果dust为ture，则返回被删除的元素 if
 * (dust) { var dustArr = []; for (var i = 0; i < this.length; i++) { if (s ==
 * this[i]) { dustArr.push(this.splice(i, 1)[0]); } } return dustArr; } for (var
 * i = 0; i < this.length; i++) { if (s == this[i]) { this.splice(i, 1); } }
 * return this; }
 */
if (!isIE && HTMLElement) {
	if (!HTMLElement.prototype.attachEvent) {
		window.attachEvent = document.attachEvent = HTMLElement.prototype.attachEvent = function(
				evtName, func) {
			evtName = evtName.substring(2);
			this.addEventListener(evtName, func, false);
		}
		window.detachEvent = document.detachEvent = HTMLElement.prototype.detachEvent = function(
				evtName, func) {
			evtName = evtName.substring(2);
...
var $topWindow = function() {
	...
	// if (parentWin.parent.document.getElementsByTagName("FRAMESET").length >
	// 0) break;
...
var Dialog = function() {
	...
	if ($("#skin").attr("splitMode") == true
		...
		if ($parentThemeDom.attr("defaultFontSize") != null) {
			this.defaultFontSize = Number($parentThemeDom
...
Dialog.prototype.attachBehaviors = function() {
	...
	if (this.Drag && topWin.Drag) {// 注册拖拽方法
		var dragHandle = topWin.$id("_Draghandle_" + this.ID), dragBody = topWin
				.$id("_DialogDiv_" + this.ID);
...
Dialog.prototype.setPosition = function() {
	...
	if (typeof this.Top == "string" && this.Top.indexOf("%") != -1) {
		...
		thisTop = this.fixedPosition ? bd.clientHeight * percentT
				- thisdialogDiv.scrollHeight * percentT : bd.clientHeight
				* percentT - thisdialogDiv.scrollHeight * percentT
				+ bd.scrollTop;
	}
	if (typeof this.Left == "string" && this.Left.indexOf("%") != -1) {
		var percentL = parseFloat(this.Left) * 0.01;
		thisLeft = ielt7 ? bd.clientWidth * percentL
				- thisdialogDiv.scrollWidth * percentL + bd.scrollLeft
...
Dialog.prototype.create = function() {
	...
	if (this.Style == "normal") {
		html = '\
  <table id="_DialogTable_{thisID}" width="'
				+ (this.Width + this.dialogWidthFix)
				+ '" cellspacing="0" cellpadding="0" border="0" style="font-size:'
				+ (this.defaultFontSize)
				+ 'px; font-family:'
				+ (this.defaultFontFamily)
				+ '; line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" onselectstart="return false;" style="-moz-user-select: -moz-none; '
				+ (this.Drag ? "cursor: move;" : "")
				+ '">\
      <td class="dialog_lt dialog_borderWidth"><div class="dialog_borderWidth"></div></td>\
      <td class="dialog_ct" ><div class="dialog_title"><span id="_Title_{thisID}" class="'
				+ (this.IconClass != "" ? this.IconClass : "")
				+ '" style="float:left;'
				+ (this.IconSrc != "" ? "background-repeat: no-repeat;background-position: 0 40%;padding-left:18px;display:inline-block;background-image:url("
						+ this.IconSrc + ")"
						: "")
				+ '">'
				+ this.Title
				+ '</span><input type="button" class="dialog_trans_icon"/></div>\
        <div id="_ButtonClose_{thisID}" onclick="fixProgress();Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" title="'
				+ this.closeButtonTitle
				+ '" class="dialog_closebtn" onmouseout="this.className=\'dialog_closebtn\'" onmouseover="this.className=\'dialog_closebtn_over\'" style=" '
				+ (this.ShowCloseButton ? "" : "display:none;")
				+ '"></div><div id="_ButtonMax_{thisID}" onclick="Dialog.getInstance(\'{thisID}\').max()" title="'
				+ this.maxButtonTitle
				+ '" class="dialog_maxbtn"  style="'
				+ (this.ShowMaxButton ? "" : "display:none;")
				+ '"></div><div id="_ButtonMin_{thisID}" onclick="Dialog.getInstance(\'{thisID}\').minHandler()" title="'
				+ this.minButtonTitle
				+ '" class="dialog_minbtn"  style="'
				+ (this.ShowMinButton ? "" : "display:none;")
				+ '"></div></td>\
    ...
    <tr valign="top">\
      ...
      <td align="center"><table width="100%" class="cccc" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          <tr id="_MessageRow_{thisID}" style="'
            ...
            <td valign="top" height="50"><table width="100%" cellspacing="0" cellpadding="0" border="0" class="dialog_bg" id="_MessageTable_{thisID}">\
                <tr>\
                  ...
                  <td align="left" style="line-height: 16px;"><div id="_MessageTitle_{thisID}" style="font-weight:bold">'
				+ this.MessageTitle
				+ '</div>\
                    <div id="_Message_{thisID}">'
				+ this.Message
				+ '</div></td>\
                </tr>\
              </table></td>\
          ...
          <tr>\
            <td valign="top"><div id="_Container_{thisID}" style="text-align:left;position: relative; width: '
				+ (this.Width + this.dialogWidthFix)
				+ 'px; height: '
				+ this.Height
				+ 'px;">\
	...
	'
				+ (function(obj) {
					...
					if (obj.URL)
						return '<iframe width="100%" height="100%" frameborder="0" style="background-color:#ffffff;border:none 0;" id="_DialogFrame_'
								+ obj.ID
								+ '" '
								+ (obj.InnerFrameName ? 'name="'
										+ obj.InnerFrameName + '"' : '')
								+ ' src="' + obj.displacePath() + '"></iframe>';
					return "";
				})(this)
				+ '\
              </div></td>\
          </tr>\
          <tr id="_ButtonRow_{thisID}" style="'
				+ (this.ShowButtonRow ? "" : "display:none")
				+ '">\
            <td height="36"><div id="_DialogButtons_{thisID}" style="text-align:'
				+ this.ButtonAlign
				+ '; border-top: 1px solid #DADEE5; padding: 8px 20px;background-color:#f6f6f6;">\
                <input type="button" style="'
				+ (this.ShowOkButton ? "" : "display:none")
				+ '"  value="'
				+ this.OkButtonText
				+ '" id="_ButtonOK_{thisID}"/>\
                <input type="button" style="'
				+ (this.ShowCancelButton ? "" : "display:none")
				+ '"  value="'
				+ this.CancelButtonText
				+ '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
';
  ...
  <table id="_DialogTable_{thisID}" width="'
    ...
    <tr id="_Draghandle_{thisID}" onselectstart="return false;" style="-moz-user-select: -moz-none; '
				+ (this.Drag ? "cursor: move;" : "")
				+ '">\
      <td ><div class="dialog_sample_top"><div style="padding: 9px 0 0 4px; float: left; "><span id="_Title_{thisID}">'
				+ this.Title
				+ '</span></div>\
        <div id="_ButtonClose_{thisID}" onclick="fixProgress();Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" class="dialog__simple_closebtn" style=" '
				+ (ielt7 ? "margin-top: 3px;" : "")
				+ (this.ShowCloseButton ? "" : "display:none;")
				+ '"></div></div></td>\
    </tr>\
    <tr valign="top">\
...
';
		...
		html = '\
  <table><tr><td><div class="dialog_shadow_content"><table id="_DialogTable_{thisID}" width="'
				+ (this.Width)
				+ '" cellspacing="0" cellpadding="0" border="0" style="font-size:'
				+ (this.defaultFontSize)
				+ 'px;font-family:'
    ...
    <tr id="_Draghandle_{thisID}" onselectstart="return false;" style="-moz-user-select: -moz-none; '
				+ (this.Drag ? "cursor: move;" : "")
				+ '">\
      <td><div class="dialog_shadow_content_top"><div style="padding: 9px 0 0 4px; float: left; "><span id="_Title_{thisID}">'
				+ this.Title
				+ '</span></div>\
        <div id="_ButtonClose_{thisID}" onclick="fixProgress();Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" class="dialog__simple_closebtn" style=" '
				+ (ielt7 ? "margin-top: 3px;" : "")
  ...
  </div><div><table cellpadding="0" cellspacing="0" width="'
				+ (this.Width + 120)
				+ '"  height="'
				+ heightValue
				+ '"><tr><td class="dialog_shadow_lt">&nbsp;</td><td class="dialog_shadow_ct">&nbsp;</td><td class="dialog_shadow_rt">&nbsp;</td></tr>\
  <tr><td class="dialog_shadow_lm" height="'
				+ (this.Height + 95 - 39 - 130)
				+ '">&nbsp;</td><td class="dialog_shadow_cm">&nbsp;</td><td class="dialog_shadow_rm">&nbsp;</td></tr>\
...
';
  ...
  <table id="_DialogTable_{thisID}" width="'
				...
				+ (this.defaultFontFamily)
				+ ';line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" style="display:none;" onselectstart="return false;" style="-moz-user-select: -moz-none; '
				+ (this.Drag ? "cursor: move;" : "")
				+ '">\
    </tr>\
          <tr>\
	...
	'
				+ (function(obj) {
					if (obj.InnerHtml)
						return obj.InnerHtml;
					if (obj.URL)
						return '<iframe width="100%" height="100%" frameborder="0" style="border:none 0;" id="_DialogFrame_'
								+ obj.ID
								+ '" '
								+ (obj.InnerFrameName ? 'name="'
										+ obj.InnerFrameName + '"' : '')
...
';
	...
	html = html.replace(/\{IMAGESPATH\}/gm, IMAGESPATH).replace(/\{thisID\}/gm,
			this.ID)
	...
	$(div)
			.bind(
					"click",
					function() {
						if ($(this).attr("AllowChangeIndex") == true
								|| $(this).attr("AllowChangeIndex") == "true") {
							maxIndex = maxIndex
									+ topWin.Dialog._dialogArray.length + 2;
							$(this)[0].style.zIndex = maxIndex;
							if ($(this).attr("MinToTask") == true
									|| $(this).attr("MinToTask") == "true") {
								var taskArray = topWin.Dialog._taskArray;
								var idx = getArrayPosition($(this).attr(
										"taskId"), taskArray);
								// 将当前taskId放在数组最后
								if (idx != -1) {
	...
	if (this.URL) {
		...
		this.innerFrameOnload = function() {
			...
			try {
				// ifame load the url ok ,hide the loader; add by shendongsheng.
				var $con1 = $(div).find(".cccc");
				$con1.unmask();
		...
		if (!isGecko) {
			...
			// this.innerFrame.contentWindow.addEventListener("load",
			// function(){self.innerFrameOnload()},
			// false);//在firefox下iframe仅能够绑定onload事件
	...
	// ifame load the url start, tyen mask the dialog to show loader; add by
	// shendongsheng.
	if (this.Style == "normal" && this.URL) {
		var $con1 = $(div).find(".cccc");
		$con1
				.mask(uncompile(quiLanguage.progressBar.title), 0, true,
						"#ffffff");
	}

	// div=$(div).find("table")[0];
	// 支持重设窗口的大小。
	/*
	 * console.log(div);
	 * 
	 * var dlg=this; div.ondragstart=function(){ return false; }
	 * 
	 * $(div).dragResize({scope:30,handles:"e",onEndResize:function(e){
	 * console.log(e,e.width,e.height,dlg.Width,dlg.Height); //return;
	 * dlg.Width=e.newWidth; dlg.dialogWidthFix=0; div.style.width = dlg.Width
	 * +dlg.dialogWidthFix +"px"; topWin.$id("_DialogTable_" + dlg.ID).width =
	 * dlg.Width + dlg.dialogWidthFix; topWin.$id("_Container_" +
	 * dlg.ID).style.width = dlg.Width +dlg.dialogWidthFix +"px"; }});
	 * //div=null;
	 */

};
Dialog.prototype.setSize2 = function(w, h) {
	if (w && +w > 20) {
		this.Width = +w;
		var setW = this.Width + this.dialogWidthFix;
		topWin.$id("_DialogTable_" + this.ID).width = setW;
		topWin.$id("_Container_" + this.ID).style.width = setW + "px";
	}
	// this.setPosition();
};
Dialog.prototype.setSize = function(w, h) {
	if (w && +w > 20) {
		this.Width = +w;
		topWin.$id("_DialogTable_" + this.ID).width = this.Width
				+ this.dialogWidthFix;
...
Dialog.prototype.max = function() {
	...
	if (tr[0].style.display == "none") {
		tr.show();
		$("#_Draghandle_" + this.ID).find(".dialog_minbtn").removeClass(
				"dialog_decreasebtn");
		$("#_Draghandle_" + this.ID).find(".dialog_minbtn").attr("title",
				this.minButtonTitle);
	}
	// 如果已经最大化，则还原
	if (this.maxFlag == true) {
		topWin.$id("_DialogTable_" + this.ID).style.width = this.Width
				+ this.dialogWidthFix;
		// topWin.$id("_Container_" + this.ID).style.width = this.Width + "px";
		topWin.$id("_Container_" + this.ID).style.width = this.Width
				+ this.dialogWidthFix + "px";
		topWin.$id("_Container_" + this.ID).style.height = this.Height + "px";
		$("#_Draghandle_" + this.ID).find(".dialog_maxbtn").removeClass(
				"dialog_decreasebtn");
		$("#_Draghandle_" + this.ID).find(".dialog_maxbtn").attr("title",
				this.maxButtonTitle);
		this.maxFlag = false;
	...
	// 最大化
		...
		// topWin.$id("_Container_" + this.ID).style.width =
		// bgDivWidth-this.dialogWidthFix + "px";
		topWin.$id("_Container_" + this.ID).style.width = bgDivWidth + "px";
		if ($("#_ButtonRow_" + this.ID)[0].style.display == "none") {// 不显示按钮栏时
			if (this.MinToTask) {
				topWin.$id("_Container_" + this.ID).style.height = bgDivHeight
						- 46 - $(".dialogTaskBg").height() + "px";
			} else {
				topWin.$id("_Container_" + this.ID).style.height = bgDivHeight
						- 46 + "px";
			}
			...
			if (this.MinToTask) {
				topWin.$id("_Container_" + this.ID).style.height = bgDivHeight
						- 46 - $("#_ButtonRow_" + this.ID).height()
						- $(".dialogTaskBg").height() + "px";
			} else {
				topWin.$id("_Container_" + this.ID).style.height = bgDivHeight
						- 46 - $("#_ButtonRow_" + this.ID).height() + "px";
			}

		}

		$("#_Draghandle_" + this.ID).find(".dialog_maxbtn").addClass(
				"dialog_decreasebtn");
		$("#_Draghandle_" + this.ID).find(".dialog_maxbtn").attr("title",
				this.decreaseButtonTitle);
		...
		this.maxFlag = true;
		this.setPosition();
		$mainDiv[0].style.left = 0;
		$mainDiv[0].style.top = 0;
	}
...
// add by wenquan 添加了窗口隐藏函数
Dialog.prototype.hide = function() {
	$("#_DialogTable_" + this.ID).css("display", "none");
};
// add by sds, 判断窗口是否可见
Dialog.prototype.isVisible = function() {
	var $dt = $("#_DialogTable_" + this.ID);
	if ($dt.length > 0) {
		var dis = $dt.css("display");
		if ("none" == dis) {
			return false;
		} else {
			return true;
...
/**
 * 获取或设置Dialog对应的zIndex
 */
Dialog.prototype.zIndex = function(zValue) {
	var div = topWin.$id("_DialogDiv_" + this.ID);
	var zVal1 = parseInt(zValue);
	if (zValue && !isNaN(zVal1)) {
		$(div)[0].style.zIndex = zVal1;
	} else {
		return parseInt($(div)[0].style.zIndex);
	}
	...
	if (tr[0] != undefined) {
		...
		if (tr[0].style.display == "none") {
			topWin.$id("_DialogTable_" + this.ID).width = this.Width
					+ this.dialogWidthFix;
			topWin.$id("_Container_" + this.ID).style.width = this.Width + "px";
			topWin.$id("_Container_" + this.ID).style.height = this.Height
					+ "px";
			$("#_Draghandle_" + this.ID).find(".dialog_minbtn").removeClass(
					"dialog_decreasebtn");
			$("#_Draghandle_" + this.ID).find(".dialog_minbtn").attr("title",
		...
		// 最小化
			...
			topWin.$id("_DialogTable_" + this.ID).style.width = 240
					+ this.dialogWidthFix + "px";
			topWin.$id("_Container_" + this.ID).style.width = 240 + "px";
			$("#_Draghandle_" + this.ID).find(".dialog_minbtn").addClass(
					"dialog_decreasebtn");
			$("#_Draghandle_" + this.ID).find(".dialog_minbtn").attr("title",
					this.decreaseButtonTitle);
			tr.hide();
...
Dialog.prototype.minToTask = function() {
	...
	if ($("#skin").attr("splitMode") == true
			|| $("#skin").attr("splitMode") == "true") {
		dialogTaskTop = window.document.documentElement.clientHeight
				- dialogTaskBottom + "px";
	} else {
		var $parentThemeDom = $(window.top.document.getElementById("theme"));
		...
		dialogTaskTop = top.window.document.documentElement.clientHeight
				- dialogTaskBottom + "px";
	}
	// 最小化到任务栏
...
// update by wenquan 修改了show的方法
Dialog.prototype.show = function() {
	var tmpTable = $("#_DialogTable_" + this.ID);
	if (tmpTable[0] != null) {
		if (tmpTable[0].style.display == "none") {
			tmpTable.css('display', 'block');
			// 将这个窗口处于所有的窗口的最上面。 shendongsheng add
			var div = topWin.$id("_DialogDiv_" + this.ID);
			if (div) {
				...
				$hiR.css(
						{
							"width" : 0,
							"height" : 0,
							"left" : $taskTargetR.find("#task_" + this.ID)
									.offset().left
									+ "px",
							"top" : dialogTaskTop
						}).animate({
	...
	this.create();
	var bgdiv = this.getBgdiv(), thisdialogDiv = this.getDialogDiv();
	...
	if (topWin.Dialog._dialogArray.length > 0) {
		...
		// topWin.Dialog._dialogArray[topWin.Dialog._dialogArray.length -
		// 1].zindex + 2;
		maxIndex = maxIndex + topWin.Dialog._dialogArray.length + 2;
	...
	// bgdiv.style.zIndex =
	// topWin.Dialog._dialogArray[topWin.Dialog._dialogArray.length - 1].zindex
	// - 1;
			...
			$task
					.bind(
							...
							function() {
								...
								var idx = getArrayPosition($(this).attr("id"),
										myTaskArray);
...
Dialog.prototype.close = function() {
	if (this.unauthorized == false) {
		if (this.innerWin && this.innerWin.Dialog
				&& this.innerWin.Dialog._childDialogArray.length > 0) {
			return;
		}
	}
	...
	if (topWin.Dialog._dialogArray.length > 0) {
		...
		/*
		 * if (this.Modal){ Dialog.bgDiv.style.display = "none"; }
		 */
		// alert(hiddenBgDiv)
	} else {
	...
	// update by wenquan 修改一个bug
	// if(this.openerWindow!=null){
...
Dialog.prototype.autoClose = function() {
	...
	this.AutoClose -= 1;
	topWin.$id("_Title_" + this.ID).innerHTML = this.AutoClose
...
Dialog.prototype.getBgdiv = function() {
	...
	if (Dialog.bgDiv) {
		if (this.Modal) {
			...
			document.getElementById("_DialogBGMask").style.filter = "alpha(opacity="
					+ opactty1 + ")";
...
Dialog.alert = function(msg, func, w, h, autoClose) {
	...
	if (msg) {
		...
		diag.InnerHtml = '<table height="100%" class=\"dlg_alertmsg_table\" border="0"  align="center" cellpadding="10" cellspacing="0">\
			<tr><td align="right"><input type="button" id="Icon_'
				+ this.ID
				+ '" class="icon_alert" align="absmiddle"></td>\
				<td align="left" id="Message_'
				+ this.ID
				+ '" style="font-size:'
				+ (diag.defaultFontSize)
				+ 'px;font-family:'
				+ (diag.defaultFontFamily)
				+ '">'
				+ msgArray[0] + '</td></tr>\
		</table>';
	} else {
		diag.Title = diag.alertTitleMessage;
		diag.InnerHtml = '<table height="100%" class=\"dlg_alertmsg_table\" border="0" class=\"dlg_alertmsg_table\" align="center" cellpadding="10" cellspacing="0">\
			<tr><td align="right"><input type="button" id="Icon_'
				+ this.ID
				+ '" class="icon_alert" align="absmiddle"></td>\
				<td align="left" id="Message_'
	...
	if (autoClose && autoClose > 0) {
		 ...
		 * setTimeout(function(){ diag.close(); },autoClose*1000)
		 */
		if (this.closed) {
			clearTimeout(this._closeTimeoutId);
			return;
		...
		this.AutoClose -= 1;
		topWin.$id("_Title_" + diag.ID).innerHTML = autoClose
				+ diag.autoCloseMessage;
		this._closeTimeoutId = setTimeout(function() {
			diag.autoClose();
		}, autoClose * 1000);
	}
...
Dialog.confirm = function(msg, funcOK, funcCal, w, h) {
	...
	diag.InnerHtml = '<table height="100%" class=\"dlg_alertmsg_table\" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><input type="button" id="Icon_'
			+ this.ID
			+ '" class="icon_query" align="absmiddle"></td>\
			<td align="left" id="Message_'
			+ this.ID
			+ '" style="font-size:'
			+ (diag.defaultFontSize)
...
// 自定义的消息确认框
// add by sds
Dialog.confirm2 = function(opt0) {
	var defOpt = {
		Msg : "",
		Title : "信息确认",
		ShowCloseButton : true,
		IconClass : "icon_query",
	...
	diag.ShowButtonRow = true;
	diag.ShowCloseButton = opt.ShowCloseButton;
	diag.Title = opt.Title;
	diag.CancelEvent = function() {
		diag.close();
		if (opt.CancelEvent && (typeof opt.CancelEvent) == "function") {
			opt.CancelEvent();
		}
	};
	diag.OKEvent = function() {
		diag.close();
		if (opt.OKEvent && (typeof opt.OKEvent) == "function") {
			opt.OKEvent();
		}
	};
	diag.ID = "confirmid";
...
function setTaskCurrent($taskTarget, taskArray) {
	...
	if (taskArray.length != 0) {
		$taskTarget.find("#" + taskArray[taskArray.length - 1]).addClass(
...
$(function() {
	if ($("#skin").attr("splitMode") == true
		...
		if ($parentThemeDom.attr("dialogTaskWidthModify") != null) {
			dialogTaskWidthModify = Number($parentThemeDom
					.attr("dialogTaskWidthModify"));
		}
		...
		if ($taskTarget[0]) {
			if ($taskTarget[0].style.display != "none") {
				if (itemTotalWidth > itemContainerWidth) {
					...
					$taskTarget.find(".taskItemContainer").css("left",
							dialogTaskStart);
				}
			}
		}
